...
# 비트마스킹을 이용해 문제를 풀어 준다.

n = int(sys.stdin.readline())
# nodes[i][j]는 i도시에서 j도시까지 가기위한 비용을 의미한다.
nodes = [list(map(int, sys.stdin.readline().split())) for _ in range(n)]
# 비트마스킹 이용

# 모든 도시를 방문했을때 visit_all과 같은값(!5)이 되어야 한다.
visit_all = (1 << n)-1
# dp[i][j]
...
def dfs(last, visited):
    ...
    for i in range(n):
        if(visited & (1 << i) == 0 and nodes[last][i] != 0):
            temp = min(temp, dfs(i, visited | (1 << i)) + nodes[last][i])
