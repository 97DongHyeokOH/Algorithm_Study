# 문제: 어린 왕자는 소혹성 B-664에서 자신이 사랑하는 한 송이 장미를 위해 살아간다. 어느 날 장미가 위험에 빠지게 된 것을 알게 된 어린 왕자는, 장미를 구하기 위해 은하수를 따라 긴 여행을 하기 시작했다.
...
# 테스트 케이스 입력값들을 받아서 결과값 출력
for _ in range(t):
    x1, y1, x2, y2 = map(int, sys.stdin.readline().split())
    n = int(sys.stdin.readline())

    # 문제 풀이: 출발점과 도착지점이 각 원안에 들어있거나 경계선에 존재한다면 집합에 i번째 원안에 들어있다는 것을 표현하기위해
    # 집합을 만들어 원안에 출발,도착점이 있다면 각 집합에 넣어준뒤 두개의 집합에서 중복되지 않는 원의 갯수가 정답이 된다.
    start = set()
    end = set()
    for i in range(n):
        ...
        if(inside(x2, y2, x, y, r)):
            end.add(i)

    result = len(start.union(end) - start.intersection(end))
    print(result)
